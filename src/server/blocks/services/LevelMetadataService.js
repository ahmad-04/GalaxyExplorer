...
export class LevelMetadataService {
     ...
     */
    static async enrichLevelData(postId, baseData) {
        try {
     ...
     */
    static async getPlayCount(postId) {
        try {
     ...
     */
    static async getAverageRating(postId) {
        try {
     ...
     */
    static async getLastPlayedTimestamp(postId) {
        try {
            ...
            console.error(`[LevelMetadataService] Error getting last played timestamp for ${postId}:`, error);
            return undefined;
        }
    }
    /**
     ...
     */
    static async getThumbnailUrl(postId) {
        try {
     ...
     */
    static createShortDescription(description) {
        const maxLength = 100;
     ...
     */
    static async shouldBeFeatured(postId, playCount, averageRating) {
        try {
            ...
            console.error(`[LevelMetadataService] Error determining featured status for ${postId}:`, error);
            return false;
        }
    }
    /**
     * Generate badge text based on level characteristics
     */
    static generateBadgeText(levelData, playCount, averageRating) {
        // New level badge
        if (playCount === 0) {
            return 'New';
        }
     ...
     */
    static getCardColorForDifficulty(difficulty) {
     ...
     */
    static async updatePlayCount(postId) {
        try {
     ...
     */
    static async addRating(postId, rating, userId) {
        try {
     ...
     */
    static async setThumbnailUrl(postId, thumbnailUrl) {
        try {
            const thumbnailKey = `level:thumbnail:${postId}`;
     ...
     */
    static async setFeaturedStatus(postId, featured) {
        try {
            const featuredKey = `level:featured:${postId}`;
            if (featured) {
     ...
     */
    static async enrichMultipleLevels(levelData) {
        try {
            const enrichedLevels = await Promise.all(levelData.map(async ({ postId, data }) => {
     ...
     */
    static async cleanupOldMetadata() {
        try {
     ...
     */
    static extractPostIdFromMetadataKey(key) {
