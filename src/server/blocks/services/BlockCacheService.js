...
export class BlockCacheService {
     ...
     */
    static async set(key, data, ttlSeconds) {
        try {
            const startTime = Date.now();
            ...
            console.log(`[BlockCacheService] Cache SET for ${key} with TTL ${ttlSeconds}s (${duration}ms)`);
        }
        catch (error) {
            await this.recordCacheError(key, error);
     ...
     */
    static async invalidateBlockCache(blockType, identifier) {
        try {
            console.log(`[BlockCacheService] Invalidating cache for ${blockType}${identifier ? ` (${identifier})` : ''}`);
            const keysToDelete = [];
            switch (blockType) {
                case 'level-preview':
                    if (identifier) {
                        keysToDelete.push(BLOCK_CACHE_KEYS.LEVEL_METADATA(identifier), BLOCK_CACHE_KEYS.BLOCK_CONFIG(identifier));
                    }
                    break;
            ...
            console.log(`[BlockCacheService] Invalidated ${keysToDelete.length} cache keys for ${blockType}`);
        }
        catch (error) {
     ...
     */
    static async invalidateRelatedCaches(blockType, identifier) {
        try {
     ...
     */
    static async batchInvalidate(keys) {
        try {
            if (keys.length === 0)
                return;
     ...
     */
    static startCacheWarming(strategy) {
        const config = {
            enabled: true,
     ...
     */
    static async resetCacheMetrics() {
        try {
            const metricsKey = 'cache:metrics:blocks';
            const resetMetrics = {
                hits: 0,
     ...
     */
    static async recordCacheHit(_key, duration) {
        try {
            const metrics = await this.getCacheMetrics();
            metrics.hits += 1;
     ...
     * Record cache miss
     */
    static async recordCacheMiss(_key) {
        try {
            const metrics = await this.getCacheMetrics();
            metrics.misses += 1;
            metrics.lastUpdated = Date.now();
     ...
     */
    static async recordCacheSet(_key, duration) {
        try {
            const metrics = await this.getCacheMetrics();
            metrics.sets += 1;
            metrics.lastUpdated = Date.now();
     ...
     */
    static async recordCacheDelete(_key, duration) {
        try {
            const metrics = await this.getCacheMetrics();
            metrics.deletes += 1;
            metrics.lastUpdated = Date.now();
     ...
     * Record cache error
     */
    static async recordCacheError(_key, _error) {
        try {
            const metrics = await this.getCacheMetrics();
            metrics.errors += 1;
            metrics.lastUpdated = Date.now();
     ...
     */
    static async recordPerformanceMetric(operation, duration) {
        try {
     ...
     */
    static async warmCommunityData() {
        try {
     ...
     */
    static async warmPopularLevels() {
        try {
            ...
            console.log(`[BlockCacheService] Warmed ${Math.min(5, levelActivity.length)} popular level caches`);
        }
        catch (error) {
     ...
     */
    static async warmActiveChallenges() {
        try {
...
BlockCacheService.warmingInterval = null;
BlockCacheService.isWarming = false;
