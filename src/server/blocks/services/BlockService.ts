...
export class BlockService {
   ...
   */
  static async storeBlockConfig(postId: string, config: BlockConfig): Promise<void> {
   ...
   */
  static async getBlockConfig(postId: string): Promise<BlockConfig | null> {
    const key = BLOCK_CACHE_KEYS.BLOCK_CONFIG(postId);
    ...
    try {
      return JSON.parse(configStr) as BlockConfig;
    } catch (error) {
   ...
   */
  static createChallengeBlockData(weekId: string): ChallengeBlockData {
    return {
   ...
   */
  static createLandingBlockData(): LandingBlockData {
    return {
   ...
   */
  static createCommunityBlockData(): CommunityBlockData {
    return {
      statistics: {
        totalLevels: 0,
        activePlayers: 0,
   ...
   */
  static async storeBlockMetadata(metadata: BlockMetadata): Promise<void> {
