...
export class WeeklyChallengeBlock {
     ...
     */
    static createChallengeInfo(challengeData) {
        return {
            type: 'challenge-info',
     ...
     */
    static createLeaderboardPreview(challengeData) {
     ...
     */
    static createTimeRemainingDisplay(challengeData) {
     ...
     */
    static createChallengeStats(challengeData) {
        const stats = [
            {
                label: 'Participants',
     ...
     */
    static createDifficultyStars(level) {
    ...
    static createEnhancedBlockConfig(config, options) {
        ...
        if (options?.showParticipantAvatars) {
            ...
            baseBlock.components[1].leaderboardPreview.entries.forEach((entry) => {
                entry.avatar = {
                    type: 'avatar',
                    username: entry.username,
                    size: 'small',
