import { CommunityBlockData, BlockConfig } from '../../../shared/types/blocks.js';
import {
...
export class CommunityShowcaseBlock {
  ...
  static createBlockConfig(config: BlockConfig): any {
    const communityData = config.data as CommunityBlockData;
   ...
   */
  private static createCommunityStats(statistics: CommunityBlockData['statistics']) {
    const stats = [
      {
        label: 'Total Levels',
   ...
   * Create featured creators section
   */
  private static createFeaturedCreators(featuredCreators: CommunityBlockData['featuredCreators']) {
   ...
   * Create popular levels carousel
   */
  private static createPopularLevels(popularLevels: CommunityBlockData['popularLevels']) {
   ...
   */
  private static createCommunityEvents(communityEvents?: CommunityBlockData['communityEvents']) {
   ...
   */
  private static getCreatorBadge(levelCount: number): string {
    if (levelCount >= 10) return '🏆'; // Master Creator
    if (levelCount >= 5) return '⭐'; // Star Creator
    if (levelCount >= 3) return '🌟'; // Rising Creator
    return '🚀'; // New Creator
  }
   ...
   */
  private static getEventStatus(startDate: number, endDate: number): string {
    const now = Date.now();
   ...
   */
  static createLeaderboardBlockConfig(config: BlockConfig): any {
    const communityData = config.data as CommunityBlockData;
    const baseBlock = this.createBlockConfig(config);
