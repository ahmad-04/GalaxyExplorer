import { LandingBlockData, BlockConfig } from '../../../shared/types/blocks.js';
import {
...
export class LandingBlock {
   ...
   */
  private static createFeaturesSection(features: string[]) {
    return {
      type: 'features',
   ...
   * Create community highlights carousel
   */
  private static createCommunityHighlights(highlights: LandingBlockData['communityHighlights']) {
   ...
   */
  private static createAppStats(statistics: LandingBlockData['statistics']) {
    const stats = [
      {
        label: 'Total Levels',
   ...
   */
  static createCTABlockConfig(config: BlockConfig): any {
    const baseBlock = this.createBlockConfig(config);
