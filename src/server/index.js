...
router.post('/api/publish-level', async (req, res) => {
    try {
        ...
        const { levelId, name, description, authorDisplay, levelData, clientPublishToken } = req.body || {};
...
router.get('/api/leaderboard', async (req, res) => {
    try {
        ...
        if (!leaderboardStr) {
            console.log(`No post-specific leaderboard found for ${postId}, checking global leaderboard...`);
...
router.post('/api/submit-test-score', async (req, res) => {
    try {
        ...
        try {
            ...
            // Also update the global leaderboard for backward compatibility
            await redis.set('leaderboard_json', JSON.stringify(trimmedLeaderboard));
            console.log('[DEV] Leaderboard updated successfully with', trimmedLeaderboard.length, 'entries');
        }
        catch (redisError) {
