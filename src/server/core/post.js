...
// Use Devvit's default dark theme (no custom background)
// For custom backgrounds, set via Redis: 'splash:backgroundUri'
async function getSplashBackgroundUri() {
  try {
    const override = await redis.get('splash:backgroundUri');
    if (override && typeof override === 'string' && /^https?:\/\//i.test(override)) {
      return override;
    }
  } catch (e) {
    // Non-fatal: fall back to default
    console.warn('[post] Failed to read splash background override, using default', e);
  }
  // Return undefined to use Devvit's default dark theme
  return undefined;
}
/**
 * Simple post creation following basic Devvit docs pattern
 */
export const createPost = async (title, properties) => {
  ...
  const backgroundUri = await getSplashBackgroundUri();
  ...
  const post = await reddit.submitCustomPost({
    ...
    splash: {
      ...
      ...(backgroundUri && { backgroundUri }), // Only include if defined
 ...
 */
export const createThemedPost = async (theme, title, properties) => {
...
export const createLevelPost = async (args) => {
  ...
  const post = await reddit.submitCustomPost({
    ...
    splash: {
      appDisplayName: 'Galaxy Explorer',
      backgroundUri:
        (args?.splash && 'backgroundUri' in args.splash && args.splash.backgroundUri) ||
        (await getSplashBackgroundUri()),
 ...
 */
export const createLandingPost = async (useBlocks) => {
  const { subredditName } = context;
  ...
  const post = await reddit.submitCustomPost({
    ...
    splash: {
      appDisplayName: 'Galaxy Explorer',
      backgroundUri: await getSplashBackgroundUri(),
 ...
 */
export const createWeeklyChallengePost = async (useBlocks) => {
  const { subredditName } = context;
 ...
 */
export const createCommunityShowcasePost = async (useBlocks) => {
  const { subredditName } = context;
  if (!subredditName)
