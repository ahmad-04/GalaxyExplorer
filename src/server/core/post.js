...
export const createLevelPost = async (args) => {
    const { subredditName } = context;
    if (!subredditName)
        throw new Error('subredditName is required to create a level post');
...
export const createLandingPost = async (useBlocks) => {
    const { subredditName } = context;
    if (!subredditName)
        throw new Error('subredditName is required to create a landing post');
    const post = await reddit.submitCustomPost({
        subredditName,
...
export const createWeeklyChallengePost = async (useBlocks) => {
    const { subredditName } = context;
    if (!subredditName)
        throw new Error('subredditName is required to create weekly post');
    const now = new Date();
...
export const createMainMenuPost = async () => {
    const { subredditName } = context;
    if (!subredditName)
        throw new Error('subredditName is required to create a main menu post');
    const post = await reddit.submitCustomPost({
        subredditName,
