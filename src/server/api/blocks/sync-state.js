...
export async function handleStateSyncRequest(request) {
    try {
        // Parse request body
 ...
 * Process state changes based on block type
 */
async function processStateChanges(syncData) {
    ...
    try {
        ...
        await BlockService.updateBlockMetadata(postId, {
            ...
            interactionCount: blockConfig.data.interactionCount + 1 || 1,
        });
...
async function processChallengeStateChanges(postId, changes) {
    try {
        ...
        if (changes.statistics?.participantCount !== undefined) {
            ...
            updated = true;
            console.log(`Updated participant count for challenge ${challengeData.weekId}: ${changes.statistics.participantCount}`);
        }
        // Update user progress
        if (changes.userProgress) {
            await BlockService.updateUserProgress(postId, changes.userProgress);
 ...
 */
async function processLandingStateChanges(postId, changes) {
    try {
